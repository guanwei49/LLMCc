...
def parse_element(bpmn_graph, counts, curr_el, parents, incoming_dict, outgoing_dict, nodes_dict, nodes_bounds,
    ...
    if tag.endswith("datastorereference"):
        id = curr_el.get("id")
        ...
        dataObjectReference = BPMN.dataStoreReference(id=id, name=name, process=process)
        bpmn_graph.add_node(dataObjectReference)
        node = dataObjectReference
        nodes_dict[id] = node
    if tag.endswith("dataobjectreference"):
        id = curr_el.get("id")
        name = curr_el.get("name").replace("\r", "").replace("\n", "") if "name" in curr_el.attrib else ""
        # name = re.sub(r'\s+', ' ', name).strip()
        # this_type = str(curr_el.tag)
        # this_type = this_type[this_type.index("}") + 1:]
        dataObjectReference = BPMN.dataObjectReference(id=id, name=name, process=process)
        bpmn_graph.add_node(dataObjectReference)
        node = dataObjectReference
        nodes_dict[id] = node
    elif tag.endswith("collaboration"):
        ...
        nodes_dict[id] = node
    elif tag.endswith("sequenceflow") or tag.endswith("messageflow"):
    ...
    elif tag.endswith("dataoutputassociation"):
        seq_flow_id = curr_el.get("id")
        for child in curr_el:
            if child.tag.endswith("targetRef"):
                name = child.get("name").replace("\r", "").replace("\n", "") if "name" in curr_el.attrib else ""
                incoming_dict[seq_flow_id] = (child.text.strip(), process, tag, name, parents)
                outgoing_dict[seq_flow_id] = (node.id, process, tag, name, parents)
    elif tag.endswith("datainputassociation"):
        seq_flow_id = curr_el.get("id")
        for child in curr_el:
            if child.tag.endswith("sourceRef"):
                name = child.get("name").replace("\r", "").replace("\n", "") if "name" in curr_el.attrib else ""
                incoming_dict[seq_flow_id] = (node.id, process, tag, name, parents)
                outgoing_dict[seq_flow_id] = (child.text.strip(), process, tag, name, parents)
        # BPMN.Association
        ...
        for seq_flow_id in outgoing_dict:
            if outgoing_dict[seq_flow_id][0] in nodes_dict:
                outgoing_dict[seq_flow_id] = (
                    nodes_dict[outgoing_dict[seq_flow_id][0]], outgoing_dict[seq_flow_id][1],
...
def read_bpmn(path):
    ...
    parse_element(bpmn_graph, counts, xml_tree.getroot(), [], incoming_dict, outgoing_dict, nodes_dict, nodes_bounds,
                  flow_info)
